...
def takeInput():
    ...
    # Normalized inputs based on training data
    try:
        age1 = ((int(age.get()) - 29) / (77 - 29))
        trestbps1 = ((int(rbp.get()) - 94) / (200 - 94))
        chol1 = ((int(serumChol.get()) - 126) / (564 - 126))
        ...
        inputValues.extend([
            age1, int(sex.get()), int(chestPain.get()), trestbps1, chol1,
            int(FBS.get()), int(ECG.get()), thalach1, int(exang.get()), 
            oldpeak1, int(slope.get()), int(ca.get()), int(thal.get())
        ])
        ...
        print("Please fill in all fields correctly with numeric values.")
